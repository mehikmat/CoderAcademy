...
import edu.ncit.datagen.tables.UserVisits;
...
public class StructuredData {
    ...
    private static final Log log = LogFactory.getLog(StructuredData.class.getName());
    private static final String RANKINGS = "rankings";
    private static final String USERVISITS = "uservisits";
    private DataGenOptions options;
    private long visits;
    ...
    public StructuredData(DataGenOptions options) {
    ...
    private void parseArgs(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if ("-v".equals(args[i])) {
                ...
                DataGenOptions.printUsage("Unknown hive data arguments -- " + args[i] + "!!!");
            }
        }
    ...
    private void createRankingsTableDirectly() throws IOException, URISyntaxException {
        ...
        JobConf job = new JobConf(StructuredData.class);
    ...
    public static class DummyToAccessNoMapper extends MapReduceBase implements
        ...
        public void configure(JobConf job) {
            try {
                ...
                userVisits = new UserVisits(DistributedCache.getLocalCacheFiles(job),
                        delim, pages);
            ...
            if (0 != vitem.ulen) {
                if (vitem.refs > 0) {
                    for (int i = 0; i < vitem.refs; i++) {
                        Text value = new Text(userVisits.nextAccess(new String(vitem.url)));

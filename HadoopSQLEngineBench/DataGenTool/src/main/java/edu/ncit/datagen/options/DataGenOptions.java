package edu.ncit.datagen.options;
...
//	private static final Log log = LogFactory.getLog(DataOptions.class.getName());
    ...
    public DataGenOptions(String[] args) throws ClassNotFoundException {
...
//		workPath = new Path(resultPath, TEMP_DIR);
    ...
    public static final int printUsage(String msg) {
        ...
        System.out.println("generate -t nutch -p <pages> [-w <search_words.txt>] "
                + "[-b <base path>] [-n <data name>] "
                + "[-m <num maps>] [-r <num reduces>] "
                + "[-o sequence] [-c <codec>]");

        System.out.println("generate -t bayes -p <pages> [-w <search_words.txt>] -g <num classes>"
                + "[-b <base path>] [-n <data name>] "
                + "[-m <num maps>] [-r <num reduces>] "
                + "[-o sequence] [-c <codec>]");
    ...
    private void checkOptions() {
        ...
        switch (type) {
            ...
            case BAYES:
                if (pages <= 0 || words <= 0) {
                    System.exit(printUsage("Error: pages/search_words.txt of bayes data should be larger than 0!!!"));
                }
                break;
            case NUTCH:
                if (pages <= 0 || words <= 0) {
                    System.exit(printUsage("Error: pages/search_words.txt of nutch data should be larger than 0!!!"));
                }
                break;
            default:

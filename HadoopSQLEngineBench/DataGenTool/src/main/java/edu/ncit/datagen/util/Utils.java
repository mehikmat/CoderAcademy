...
public class Utils {
    ...
    public static final void shareDict(DataGenOptions options, JobConf job) throws URISyntaxException {
    ...
    public static final void shareUrls(String symbol, DataGenOptions options, JobConf job) throws IOException, URISyntaxException {
    ...
    private static final void shareZipfCore(String fname, DataGenOptions options, JobConf job) throws URISyntaxException {
    ...
    public static final void shareLinkZipfCore(DataGenOptions options, JobConf job) throws URISyntaxException {
        shareZipfCore(HtmlCore.LINK_ZIPF_FILE_NAME, options, job);
    }

    public static final void shareWordZipfCore(DataGenOptions options, JobConf job) throws URISyntaxException {
        shareZipfCore(HtmlCore.WORD_ZIPF_FILE_NAME, options, job);
    }

    public static final void serialLinkZipf(DataGenOptions options) throws IOException {
        if (options.getNumPages() > 0) {
            Zipfian lzipfian = new Zipfian(options.getNumPages(), DataGenOptions.LINK_ZIPF_EXPONENT);
            lzipfian.setupZipf((long) (options.getNumPages() * HtmlCore.getMeanLinksPerPage()), DataGenOptions.LINK_SIMULATE_SPACE_RATIO);
    ...
    public static final void serialWordZipf(DataGenOptions options) throws IOException {
        if (options.getNumWords() > 0) {
            Zipfian wzipfian = new Zipfian(options.getNumWords(), DataGenOptions.WORD_ZIPF_EXPONENT);
            wzipfian.setupZipf((long) (options.getNumPages() * HtmlCore.getMeanWordsPerPage()), DataGenOptions.WORD_SIMULATE_SPACE_RATIO);
            Path wzipfCorePath = new Path(options.getWorkPath(), HtmlCore.WORD_ZIPF_FILE_NAME);
            serialZipfCore(wzipfian, wzipfCorePath);
        } else {
            System.out.println("ERROR: number of search_words.txt should be greater than 0");
            System.exit(-1);
        }
    }
}

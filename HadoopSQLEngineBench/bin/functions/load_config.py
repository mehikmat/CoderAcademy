...
def check_config():  # check configures
    ...
    for _, prop_name in HiBenchEnvPropMappingMandatory.items() + HiBenchEnvPropMapping.items():
        assert "${" not in HibenchConf.get(prop_name,
                                           ""), "Unsolved ref key: %s. \n    Defined at %s:\n    Unsolved value:%s\n" % (
                    ...
                    if matched_keys:
                        ...
                        if not "*" in k:
                            ...
                            new_key = splited_value[
                                          0] + "".join(
                                [matched_keys.groups()[idx] + x for idx, x in enumerate(splited_value[1:])])
        ...
        if not (HibenchConf.get("hibench.spark.master", "")):
            ...
            # spark standalone mode
                ...
                try:
                    ...
                    with closing(
                            urllib.urlopen('http://%s:%s' % (HibenchConf['hibench.masters.hostnames'], master_port),
                                           proxies={})) as page:
                        worker_hostnames = []
                        for x in page.readlines():
                            if worker_port in x and "worker" in x:
                                worker_hostnames.append(
                                    re.findall("http:\/\/([a-zA-Z\-\._0-9]+):%s" % worker_port, x)[0])
                        HibenchConf['hibench.slaves.hostnames'] = " ".join(worker_hostnames)
